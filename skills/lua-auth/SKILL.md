...
Tell the user to open a terminal outside Codex and run:
...
The CLI writes the issued typed personal credential to `~/.lua-cli/credentials` with mode `0600`. Never ask the user to paste an email, OTP, or credential into the Codex conversation. Never run `lua auth configure` on the user's behalf.
