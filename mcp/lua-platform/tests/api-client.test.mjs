...
describe('apiRequest', () => {
  ...
  test('identifies direct requests as the versioned Codex plugin', async () => {
    const fetchFn = mockFetch(jsonResponse({ ok: true }));
    await apiRequest('/agents', { fetchFn });
    expect(fetchFn.calls[0].init.headers['X-Lua-Client']).toBe(`codex-plugin/${PLUGIN_PACKAGE.version}`);
  });
